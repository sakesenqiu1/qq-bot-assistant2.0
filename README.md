...
> - **v0.5 禁言去重（已处罚言论不重复禁言）+ 主动检查（定时自动审查新违规并禁言）**
...
- **禁言去重**：每条群消息有唯一 ID，被处罚过的言论会记入「已处罚账本」（`data/mutes/`，与审计账本同步保留 3 天），后续 /查违规 和主动检查都会跳过，同一个人不会因为同样的几条发言被重复禁言；只有新的违规才触发新禁言
